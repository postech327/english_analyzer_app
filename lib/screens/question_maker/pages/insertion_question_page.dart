...
class _InsertionQuestionPageState extends State<InsertionQuestionPage> {
  ...
  Future<void> _generateInsertion() async {
    ...
    try {
      ...
      // ✅ 2) 서버 실패 시 폴백
      final fb = _svc.fallbackInsertion(passage: txt);
      setState(() => _items = fb);
  ...
  Widget _buildInsertionCard(McqItem q) {
    final insertSentence = (q.meta['insert_sentence'] ?? '').toString().trim();
        ...
        child: Column(
          ...
          children: [
            ...
            if (marked.isNotEmpty) ...[
              const Text('지문(삽입 위치 표시)',
                  style: TextStyle(fontWeight: FontWeight.w800)),
              const SizedBox(height: 6),
              Text(marked),
              const SizedBox(height: 12),
            ],
            const Text('선택지', style: TextStyle(fontWeight: FontWeight.w800)),
            const SizedBox(height: 6),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('문장 삽입 (ID: ${_analysisId ?? "-"})'),
        actions: [
          IconButton(
            tooltip: '저장',
          ...
          children: [
            ...
            Row(
              children: [
                const Spacer(),
                FilledButton.icon(
                  onPressed: _busy ? null : _generateInsertion,
                  icon: _busy
                      ? const SizedBox(
                          width: 16,
                          height: 16,
            ...
            if (_items.isEmpty)
              ...
              ..._items.map(_buildInsertionCard),
          ],
        ),
      ),
    );
