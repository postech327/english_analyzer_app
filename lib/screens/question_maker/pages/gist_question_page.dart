...
class _GistQuestionPageState extends State<GistQuestionPage> {
  ...
  Future<void> _generateGist() async {
    ...
    try {
      // ✅ 1) 허브 있으면: 서버 호출 없이 고정 생성 (정답=hub.gist_en)
      final hubGist = _hubValue('gist_en').trim();
      if (hubGist.isNotEmpty) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('요지 문제 (ID: ${_analysisId ?? "-"})'),
        actions: [
          IconButton(
            tooltip: '저장',
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const Spacer(),
                FilledButton.icon(
                  onPressed: _busy ? null : _generateGist,
                  icon: _busy
                      ? const SizedBox(
                          width: 16,
                          height: 16,
